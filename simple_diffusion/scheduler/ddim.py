...
class DDIMScheduler:
    ...
    def generate_with_image(
        ...
        device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        image = self.add_noise(image, torch.randn(
            image.shape,
            generator=generator,
        ).to(device), torch.LongTensor([steps_to_do]).to(device).repeat(image.shape[0]))
        
        self._set_timesteps(num_inference_steps)
        piece_size = image.shape[2:]

        for t in tqdm(self.timesteps[-steps_to_do:]):
            image = image.repeat(1, 1, (piece_size[0] + model.sample_size - 1) // (piece_size[0]),
                                 (piece_size[1] + model.sample_size - 1) // (piece_size[1]))

            b, c, h, w = image.shape
            h0 = (h - model.sample_size) // 2
            w0 = (w - model.sample_size) // 2
            image = image[:, :, h0: h0+model.sample_size, w0: w0+model.sample_size]
            ...
            image = image[:, :, h1: h1+piece_size[0], w1: w1+piece_size[1]]
